...
- [Creation Pattern](#creation-pattern)
  ...
  * [Abstract Factroy Pattern](#abstract-factroy-pattern)
...
public class Cat {
    ...
    public CatMemento createMemento() {
        CatMemento  cm = new CatMemento(this.age, this.height);
        return cm;
    }

    public void restore(CatMemento memento) {
...
- 중재자 패턴
- 오브젝트간 결합도를 낮춰주기위해 사용함 ( 직접적 연결을 줄여줌)
- 채팅 응용프로그램과 같은 메시지 기반 시스템에서 일반적으로 사용됨



- 코드 출처 : [[자바 디자인 패턴] 행동패턴 - 중재자 패턴](https://xzio.tistory.com/853)

![중재자](https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2F2zRGh%2FbtqZ9BGNBft%2Fi7efXmTzoNtvjNOg9YCRD1%2Fimg.png)
...
출처: [[자바 디자인 패턴] 행동패턴 - 중재자 패턴](https://xzio.tistory.com/853) [코딩창고]
...
- 방문자 패턴
- 오브젝트와 Visitor를 연결하여 Visitor에 있는 메소드를 사용함
  - 데이터와 데이터를 처리하는 알고리즘을 분리하고 싶을 때 사용함
  
  - 데이터 구조는 변하지 않는데 알고리즘이 자주 바뀐다면 사용해볼만 하다.
...
- 반복자 패턴
- 컬렉션의 구조를 모두 파악하고 있지않더라도 컬렉션을 사용할 수 있게 해줌
- '반복'기능에 특화되어있음
- 자바에서도 Iterator를 써본사람이 있다면 이해가 편할 것이다.
